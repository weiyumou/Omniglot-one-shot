...
if __name__ == '__main__':
    ...
    # batch, labels = next(triplet_dataloaders["val"].__iter__())
    # batch = torch.reshape(batch, (batch_sizes["val"], 3, *batch.size()[1:]))
    # labels = torch.reshape(labels, (batch_sizes["val"], -1))
    # anc, pos, neg = (torch.squeeze(x, dim=1) for x in torch.chunk(batch, 3, dim=1))
    ...
    #
    # datasets.display_image(anc[0], train_dataset.indices_to_labels[anc_labels[0].item()])
    # datasets.display_image(pos[0], train_dataset.indices_to_labels[pos_labels[0].item()])
    # datasets.display_image(neg[0], train_dataset.indices_to_labels[neg_labels[0].item()])

    # labels = [train_dataset.indices_to_labels[idx]
    #           for idx in (anc_label.item(), pos_label.item(), neg_label.item())]
    # datasets.visualise_triplet(anc[0], pos[0], neg[0], labels)
